...
__IO uint16_t ADC3_ConvertedValue[NOFCHANEL] = {0};
/**
  * @brief  ADC GPIO 初始化
  * @param  无
  * @retval 无
...
// ADCx_2 GPIO 初始化
	ADCx_3_GPIO_APBxClock_FUN ( ADCx_3_GPIO_CLK, ENABLE );
	GPIO_InitStructure.GPIO_Pin = ADCx_3_PIN;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AIN;
...
static void ADC3_Mode_Config(void)
{
  ...
  /* ------------------DMA模式配置---------------- */	
	...
	DMA_Init(ADC3_DMA_CHANNEL, &DMA_InitStructure);	
	// 使能 DMA 通道
	...
	RCC_ADCCLKConfig(RCC_PCLK2_Div8); 	
	// 配置ADC 通道的转换顺序和采样时间
	ADC_RegularChannelConfig(ADCx_3, ADCx_3_CHANNEL, 1, 
	                         ADC_SampleTime_239Cycles5);	
	// 使能ADC DMA 请求
	ADC_DMACmd(ADCx_3, ENABLE);
...
//	RCC_APB2PeriphClockCmd(ADCx_1_GPIO_CLK, DISABLE);	
